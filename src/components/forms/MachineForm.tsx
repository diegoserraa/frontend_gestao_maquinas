...
function validate(
  ...
  if (
    !form.setor_id ||
    !sectors.find((s) => s.id === form.setor_id)
  ) {
  ...
  if (
    !form.intervalo_manutencao_dias ||
    form.intervalo_manutencao_dias < 1
  ) {
  ...
  if (!form.ultima_manutencao) {
    errors.ultima_manutencao =
      "Informe a última manutenção.";
  }
...
export function MachineForm({
  ...
  const [form, setForm] = useState<MachineFormData>({
    ...
    intervalo_manutencao_dias:
      initialData?.intervalo_manutencao_dias ?? 90,
    ultima_manutencao:
      initialData?.ultima_manutencao
  ...
  const [touched, setTouched] =
    useState<
      Partial<
        Record<keyof MachineFormData, boolean>
      >
    >({});
